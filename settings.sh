...
## Uncomment the following line to set a custom site title (defaults to the project name taken from the project directory):
# WP_SITE_TITLE='Example Title'


## Uncomment the following line to set a DB search and replace post-import-db hook (update the URLS too):
...
INSTALL_RAY_CONNECTIONS=true # Install Spatie Ray connection files to work with Docker containers
INSTALL_WP_CLEAN=true # Install clean version of WP (individual settings can be set below)
INSTALL_WP_CONFIG_HOOKS=true # Add WP configurations (individual settings can be set below)
